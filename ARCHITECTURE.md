...
Conpositor has a wrapper around the zlua zig library to ensure consistent bindings, LuaContext.zig implements all of the binding system.

### Lua Objects

Objects in lua are created using [type_gen.lua](conpositor/lua/type_gen.lua) the _GenerateType function is called by Conpositor to convert tables to a proper function table. Instances are manually hashed, this allows for client references in lua to be created and destroyed without the zig instance being destroyed. If a type needs mixins it should be defined in a `[type]_mixin.lua` file. These should be properly documented as docgen.zig will parse the emmylua into its type definitions.

### Ownership model

Things owned by Conpositor should be freed by Conpositor, things owned by lua should implement a free in __gc. Types should not have instances owned by one or the other. Another important contract conpositor holds is that conpositor objects remain in scope an enitire lua call, in other words they cannot be freed by exposed functions.